  ...
  (let [required-js-names
        ...
        (reduce
          (fn [js-mods {:keys [module-id output-name depends-on sources goog-base] :as mod}]
            (let [js-mod (JSChunk. (name module-id))
...
(defn compile-js-modules
  ...
  (let [js-mods
    ...
    ;; this pass looks for shadow$export calls and replaces them with actual exports
    ;; need to do this after compilation since the closure ESM output pass runs last
    ;; we cannot do this via a custom pass since there is no stage we can inject to that runs after the ESM pass
    ;; adding exports any earlier and the closure compiler kill it
    ...
    (when (= :esm (get-in state [:compiler-options :chunk-output-type]))
      (let [pass (ShadowESMExports. compiler)
            root (.getRoot compiler)]
        (.process pass (.getFirstChild root) (.getSecondChild root))))
