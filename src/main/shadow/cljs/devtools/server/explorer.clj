...
(comment
  ;; FIXME: this entire things needs more thought
  ;; it does very little useful things and most of it should be generic helper functions
  ;; so they can also be used from the REPL
  ...
  (defn start [system-bus]
    (let [cljs-watch
          ...
          (util/server-thread
            ...
            (-> (cljs/init-state)
                (assoc
                  ::worker-impl/compile-attempt 0
                  ;; FIXME: log somewhere
                  :logger util/null-log
                  ;; race condition accessing the cache file when this starts up in parallel to something else
                  :manifest-cache-dir
                  (doto (io/file "target" "shadow-cljs" "explorer" "manifest-cache")
                    (io/make-parents))
                  :cache-dir
                  (doto (io/file "target" "shadow-cljs" "explorer" "cache")
                    (io/make-parents)))
