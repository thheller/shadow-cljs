(ns shadow.cljs.ui.views.runtimes
  (:require
...
(defc build-lang
  ...
  (render
    (case lang
      :cljs
      (<< [:svg.flex-shrink-0.h-5.w-5.text-gray-400.group-hover:text-gray-500 {:viewBox "0 0 24 24" :version "1.1" :xmlns "http://www.w3.org/2000/svg" :xmlns:xlink "http://www.w3.org/1999/xlink" :aria-hidden "true"}
...
(defn ui-runtime-listing [runtimes]
  (<< [:table.min-w-full.divide-y.divide-gray-200
       [:thead
        [:tr
         [:th.px-6.py-3.bg-gray-50.text-left.text-xs.font-medium.text-gray-500.uppercase.tracking-wider "build"]
         [:th.px-6.py-3.bg-gray-50.text-right.text-xs.font-medium.text-gray-500.uppercase.tracking-wider "host"]
...
(defc ui-runtime-overview-mobile
  [ident]
  (bind {:keys [runtime-id runtime-info supported-ops] :as data}
    (sg/query-ident ident
      [:runtime-id
...
(defn ui-runtime-listing-mobile
  ...
  (<< [:ul.mt-2.divide-y.divide-gray-200.overflow-hidden.shadow.sm:hidden
       (sg/render-seq runtimes identity ui-runtime-overview-mobile)]))
